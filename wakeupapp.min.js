    ...
    var WakeUpApp = function () {
        ...
        _createClass(WakeUpApp, [{
            ...
            value: function init() {
                ...
                // 合并下用户输入的配置
                this.config = _extends({}, Config, userConfig);
            ...
            value: function yingyongbaoOpen() {
                var url = this.config.yingyongbaoUrl + '&android_schema=' + this.getAndroidSchemaUrl();
                if (this.os === 'ios') {
            ...
            value: function callDownLoad(isAndroid) {
                ...
                var hidden = void 0;
                var self = this;
                var eventArr = ['baiduboxappvisibilitychange', 'pagehide', 'visibilitychange', 'mozvisibilitychange', 'msvisibilitychange', 'webkitvisibilitychange'];
                ...
                if (!this.config.noDownload) {
                    var url = this.config.iosDownUrl || this.config.defaultDownloadUrl;
                    if (isAndroid) {
                    ...
                    if (url) {
                        timer = setTimeout(function () {
                            var now = Date.now();
                            var timeDiff = (now - self.lastDate) / 1000;
                            if (timeDiff > 2.1) {
                                return;
                            }
                            location.href = url;
