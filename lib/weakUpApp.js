...
class WeakUpApp {
    ...
    universalLinkOpen() {
        ...
        //     newUrl = localHref.replace(reg, 'https://wapp.baidu.com');
        // } else {
        //     newUrl = localHref.replace(/^(?:http|https):\/\/[a-z.]+?\.baidu\.com/, 'https://tieba.baidu.com');
        // }
        // // test
        // newUrl = localHref.replace(/^(?:http|https):\/\/(localhost|172.22.148.41):1234/, 'https://tieba.baidu.com');

        // let otherParamStr = `${location.search ? '&' : '?'}${this.config.ios9DownUrl}&obj_locate=${this.config.locate}&obj_source=${this.config.obj_source}&obj_param2=${this.browser}&bdid=${this.bdid}`;
        // location.href = newUrl + otherParamStr;
    }
    /**
     * 如果在微信里就来应用宝
     */
