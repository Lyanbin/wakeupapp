...
class WeakUpApp {
     ...
     */
    universalLinkOpen(targetUrl) {
        let ownStatParam = `obj_locate=${this.config.locate}&obj_source=${this.config.obj_source}&obj_param2=${this.browser}&bdid=${this.bdid}`;
        if (targetUrl) {
            let host = location.host === 'tieba.baidu.com' ? 'wapp.baidu.com' : 'tieba.baidu.com';
            let params = location.search ? `${location.search}&${ownStatParam}` : `?${ownStatParam}`;
            return `${location.protocol}//host${location.pathname}${params}`;
        }
